...
15. One self-managed public-edge doctrine where MetalLB exposes Envoy Gateway, Kubernetes Gateway
    ...
    remains the
    identity provider, every externally reachable app or dashboard lives under the substrate's
...
handoff-unobserved. Sprint 2.75 is planned and next, with implementation not begun; development is
paused at that handoff. Sprints 2.54 through 2.74 are closed as recorded in their own rows.
Phase 3 is reclosed on Sprint 3.43: the
...
| 5 | Canonical Test Suite | ✅ **Reclosed on Sprint `5.36` (2026-08-23) with the descriptor-bound validation cleanup client.** `TestRunner` now selects exact registered keys and enters the authenticated descriptor-bound `ExplicitPerRun` lifecycle client before its first AWS mutation. The lifecycle compiler, Authority run, closed ordinary dispatcher, exact observers, and terminal node-state decision replace the validation-owned callback graph, operation-ID allocator, direct Route 53 sweep, ambient absence fixture, and success fold. Same-prefix restart adopts the existing nonterminal run; operational credential deletion remains the lifecycle-gated legacy tail pending Sprint `6.5`. Sprint `5.37` previously closed the configuration-ownership expansion: generated harness config takes externally chosen host/contact fields from the generated `TestSecrets` schema, cluster shape plus Vault/MinIO endpoints from a validated topology variant, and stable run-derived cluster/storage identities. The legacy aggregate carries equivalent explicit fixture fields. Both lanes share the production builder, validate before one atomic Tier-0 write, preserve a complete operator sibling, and carry a source gate against retired real Haskell deployment literals. Deployment qualification remains pending. Prior: ✅ **Reclosed on Sprint `5.34` (2026-08-13)** — an own-surface reopen closing this phase's two remaining unowned rows. The Tier-0 write gate widens from one line to one hop within one top-level definition (two drafts produced false positives first; all are recorded), and the test-secrets credential guard is one-sided because the symmetric rule is forbidden by the repository's own credential scanner. Prior: ✅ **Reclosed 2026-08-11 on Sprints `5.32`/`5.33` (Standard A/N)** — the own-surface reopen on the suite content this phase owns (Standard M) closes. `5.32`: the `LCPC-2026-07-11` reproducer consumes a repository-owned frozen trace whose per-mechanism dispositions are parsed totally over the mechanism enumeration and bound into the trace digest, and a committed mutation fixture makes the command exit non-zero. The mutation fixture is deliberately not digest-pinned, because pinning it would make the digest gate fire first and the disposition consumption — the thing under test — would never run. `5.33`: `daemon-bootstrap`'s unset arm probes the Bootstrap Broker's route surface with read-only `GET`s and refuses, naming the absent daemon, when nothing answers; the audit block declares `AUDIT_PROVENANCE=`. `gateway-partition` renders its values from the composition and left the integration surface for the unit suite, reducing the canonical node set and no coverage. Both Deployment Qualification rows were already `pending`, so no `proven` claim is retracted; the Counterexample column may now be filled by a qualification run. Prior state — ✅ Reclosed on Sprint `5.31` (2026-08-09). The final fixture drifts are corrected and the installed integration surface is green; clean-room deployment qualification remains pending under Standards O/P. | Final code-local closure: `prodbox dev check` exit 0 with the repository-pinned Fourmolu/HLint and all-target `-Werror` build; unit **4580/4580**; installed `teardown-recovery` exit 0 over 80 interruption rows and seven typed fake boundaries; installed `cli` **62/62** and `env` **62/62**; docs lint/check and diff check exit 0. Live home/AWS campaigns remain non-blocking Standards-O/P evidence. |
