...
| Lifecycle-provider AWS identity | `secret/aws/lifecycle-provider` (KV) | `policy/aws-lifecycle-provider` | fenced Provider Worker only; `Operational`, and may assume only the sole registered role deterministically selected by its committed intent. The canonical role-policy digest revisions the durable provisioning scope, so policy drift advances through a successor operation/Target generation and exact IAM read-back without reopening a completed receipt. |
