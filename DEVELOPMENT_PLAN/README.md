...
**Current head state (2026-08-10, third entry — Phases `2` and `3` reopen on their own surfaces;
...
**Two code sprints are registered, not landed.** Sprint `3.34` 📋 (Phase `3` own-surface reopen)
gives the coordinate one owner derived from `endpoints/kubernetes` — one observation yielding both
post-DNAT halves — and extends the chart lint's region to every repo-owned template. Sprint `2.42` 📋
(Phase `2` own-surface reopen) stops the broker discarding its typed transport failure, which is why
the outage cost eight runs to diagnose rather than being self-evident from the readiness body.
...
All code-owned phase surfaces are closed. The 2026-08-09 own-surface reopen of Phase `4` reclosed on
2026-08-10 with Sprints `4.73`–`4.75`, leaving no unowned `Pending Removal` row on a Phase-`4`
surface. Historical reopens and reclosures remain recorded in the
rows below; forward-only blockers follow Standards A/N, and clean-room deployment qualification
remains a distinct, pending Standards O/P axis.
...
**Phases `2` and `3` are open on their own surfaces (Standard A/N); the other seven are closed.** A
...
Service port while policy is evaluated post-DNAT against the endpoint port. Sprint `3.34` 📋 owns the
coordinate and the chart-lint region; Sprint `2.42` 📋 owns the broker's discarded transport failure.
