...
| Local Kubernetes substrate | RKE2 | Local host | `prodbox rke2 install|delete --yes|status|start|stop|restart|logs`, including summary-oriented delete reporting | RKE2 data dirs and systemd units |
...
| Chart runtime | `prodbox charts list|status <chart>|deploy <chart>|delete <chart> [--yes]` | Manage the supported `gateway`, `keycloak`, `vscode`, `api`, and `websocket` chart surfaces, with `gateway` remaining the separate distributed gateway daemon path and the shared-host browser, API, WebSocket, Harbor, and MinIO routes staying behind Envoy with Keycloak-backed auth and RBAC |
...
| Clean-room handoff | `prodbox rke2 delete --yes`, a rerun that starts and finishes with no supported-path `prodbox-config.json` artifact, `prodbox rke2 install`, `prodbox config show`, `prodbox config validate`, AWS-backed validation, `prodbox test all`, `prodbox host public-edge`, and repository review gates that confirm no supported-path placeholder-domain or Python residue survives |
