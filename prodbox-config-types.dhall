...
in  { SecretRef
    , Type =
        ...
        , aws_substrate : { hosted_zone_id : Text, subzone_name : Text }
        , ses :
            { sender_domain : Text
            , receive_subdomain : Text
            , capture_bucket : Text
            }
        , domain : { demo_fqdn : Text, demo_ttl : Natural }
        , acme :
            { email : Text
            , server : Text
        ...
        , pulumi_state_backend :
            { bucket_name : Text, region : Text, key_prefix : Text }
        }
    , default =
      { aws =
        { access_key_id =
      ...
      , ses =
        { sender_domain = "", receive_subdomain = "", capture_bucket = "" }
      ...
      , acme =
        ...
        , eab_key_id = Some
            ( SecretRef.Vault
                { mount = "secret", path = "acme/eab", field = "key_id" }
            )
        , eab_hmac_key = Some
            ( SecretRef.Vault
                { mount = "secret", path = "acme/eab", field = "hmac_key" }
            )
        }
      , deployment =
        { dev_mode = True
