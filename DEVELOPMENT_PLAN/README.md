...
**Current head state (2026-08-15, latest — teardown MISU correction).** Phases `3`, `4`, and `5`
are 🔄 Active on Sprints `3.41`, `4.84`, and `5.35`; Phase `4` also has `4.85` blocked by `4.84`
and `4.86` blocked by `3.41` + `4.85`, while Phase `5` has `5.36` blocked by `4.85` + `5.35`.
Phases `6` and `7` are ⏸️ Blocked on Sprints `6.5` and
`7.36` by valid earlier-phase prerequisites. Phases `0`, `1`, `2`, and `8` remain closed.
...
| 3 | 🔄 Active | `3.41` bootstrap-owned teardown recovery profile and caller identity |
| 4 | 🔄 Active | `4.84` exact keyed algebra; `4.85` blocked by `4.84`; `4.86` blocked by `3.41` + `4.85` |
| 5 | 🔄 Active | `5.35` frozen reference oracle; `5.36` blocked by `4.85` + `5.35` for the `TestRunner` lifecycle client |
| 6 | ⏸️ Blocked | `6.5`, blocked by `4.86` + `5.36`, owns public generic/home sole-writer cutover |
| 7 | ⏸️ Blocked | `7.36`, blocked by `4.85`, owns exact AWS/EKS adapters and prerequisite live proof |
...
**Current authoritative status (2026-08-15): Phases `3`, `4`, and `5` are Active; Phases `6` and
`7` are Blocked by their explicit earlier-phase prerequisites; Phases `0`, `1`, `2`, and `8` are
closed.** The execution order is `3.41` and `4.84` → `4.85` → `4.86`; `5.35` proceeds on its
independent frozen-reference-oracle surface, then `5.36` consumes `4.85` + `5.35`; `6.5` consumes
`4.86` + `5.36`; and `7.36` consumes
`4.85`. Live home/AWS destructive campaigns are non-blocking qualification evidence and do not
substitute for phase-local validation. They also cannot authorize public activation or legacy
deletion until the matching current-revision qualification row is `proven`.

The old cascade remains Pending Removal and is not an accepted recovery path. An incomplete
