...
**2026-07-02 — Phase 3 Sprint `3.21` partially landed and remains blocked on broker
protocol generation.** Sprint `2.27` no longer blocks this work: `src/Prodbox/Pulsar/Codec.hs`,
...
canonical CBOR payload round trips, `topicFor`, `WorkCommand`/`WorkEvent`/`WorkResult`, explicit
`PulsarBrokerProtocolUnavailable` broker-I/O guards, and gateway deployment-plan rendering of the
...
`./.build/prodbox test integration cli` (39/39), `./.build/prodbox test integration env` (39/39),
and `./.build/prodbox dev lint chart`. The sprint is **not closed**: replacing the guard with real
Apache Pulsar native broker I/O is blocked on a GHC-9.12.4-compatible generated `BaseCommand` /
`PulsarApi.proto` layer; the published `supernova` package remains incompatible (`base < 4.14`).
...
has since landed the local Pulsar CBOR/topic/chart boundary but remains blocked on broker protocol
generation for live native Pulsar I/O.
...
`protobuf`/`Lint.Proto`/`payloadJson`/`payload_json` (no hits), and `./.build/prodbox dev check`.
...
generalizes. Structured payloads are unified on canonical **CBOR project-wide** (the former
...
broker I/O still blocked on generated `BaseCommand`),
...
while broker I/O remains blocked on generated `BaseCommand`, and Sprint `4.34` has landed the pure
