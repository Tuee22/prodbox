...
**Current head state (2026-08-11 — Phases `4` and `5` reopen on their own surfaces; a lifecycle
...
role/transport isolation, deterministic IAM/controller ownership, target-local DNS01, registered
public-A reconciliation, and the code-local fault model (unit 3040/3040, installed integration
55/55 twice, `dev check` exit 0). Sprint `6.4` closes the versioned clean-room
...
**Phases `4` and `5` are 🔄 reopened on their own surfaces (2026-08-11, Standards A/N)** — Phase `4`
on Sprints `4.76`/`4.77` 📋 for the destructive-path observation folds, the fail-closed tag sweep, two
AWS argv builders, and an inert `--yes`; Phase `5` on Sprints `5.32`/`5.33` 📋 for three named
validations that cannot fail. Neither reopen blocks the other, and neither blocks any earlier phase.
Every other code-owned phase surface remains closed; the 2026-08-09 own-surface reopen of Phase `4`
reclosed on 2026-08-10 with Sprints `4.73`–`4.75`. Historical reopens and reclosures remain recorded
in the rows below; forward-only blockers follow Standards A/N, and clean-room deployment
qualification remains a distinct, pending Standards O/P axis.
...
| 7 | AWS Substrate Foundations | ✅ **Reclosed on Sprint `7.33` (2026-08-02).** Role/transport isolation, target-local DNS01, deterministic IAM/controller ownership, exact public-A Provider intents, and fault dispositions are code-locally validated; live AWS evidence remains Standard O/P. Own-surface reopen on Sprint `7.35` (2026-08-03) redacts the two real per-run cloud resource ids recorded in this phase's own live-run narrative and completes the bootstrap-credential registration in `vault_doctrine.md` §6.1. | AWS Broker/Target-Agent/Gateway parity, exact client transport to the single retained home authority, resource isolation, prerequisite fault evidence, and per-run postflight residue narrowing |
...
**The `LCPC-2026-07-11` reproducer is non-falsifying at this revision (recorded 2026-08-11).**
Standard P's counterexample rule requires a repository-owned reproducer that records an *expected
failure against the frozen superseded implementation*. The named reproducer is
`simulateFrozenCounterexample` in `src/Prodbox/Test/Qualification/FrozenCounterexample.hs`, and it
discards its `FrozenCounterexampleTrace` argument to a `_` wildcard and returns a constant built by
its own `simulateComposition`; `src/Prodbox/Test/CounterexampleValidation.hs` then asserts exactly
what that constant contains. Both the five superseded failures and the five replacement closures are
therefore produced by the module that checks them, and no input can make the node fail. Because both
rows above are already `pending`, **no `proven` claim rests on it and nothing is retracted** — but
the Counterexample/fault-matrix column of either row **must not be filled** until Sprint `5.32` ✅
lands, because a green result from this node is not evidence. This is the exact failure the section
header excludes by name ("not inferred from … fake-interpreter evidence"), reached through the
mechanism meant to enforce it. Sprint `5.32` owns the fix and its acceptance criterion is the
deliverable: mutate the frozen trace fixture, and the node must fail.
...
than an earlier one. Sprint `2.42` changes a readiness *reason string* and neither the verdict nor
any manifest, and moves no enumerated surface. Sprint `0.26` changes governed documentation only —
though note that `SourceIdentity` binds governed documentation, so a doctrine edit still moves the
source-manifest digest a future `proven` row would bind. Separately, and more concretely than the
standing `pending` status: **the AWS-substrate suite currently cannot complete**, being blocked at
the `bootstrap-broker` release by the defect Sprint `3.34` owns.
...
**All nine phases are closed on their code-owned surfaces (2026-08-11).** A
live `prodbox test all --substrate aws` run on 2026-08-10 failed at the `bootstrap-broker` Helm
release eight consecutive times, on a chart NetworkPolicy that permits Kubernetes API egress on the
Service port while policy is evaluated post-DNAT against the endpoint port. Sprint `3.34` ✅ **Done (2026-08-11)** owned the
coordinate and the chart-lint region and recloses Phase `3`; Sprint `2.42` ✅ **Done (2026-08-11)** owned the broker's
discarded transport failure. Fixing the chart defect exposed three further broker-readiness defects it
had masked, landed as Sprint `2.43` ✅ **Done (2026-08-11)** on Phase `2`'s own surface. The remaining
open axis is Standard-O/P: a live run proving the broker reaches ready, and deployment qualification.
Governance Sprint `0.26` ✅ landed the doctrine both implement, on the already-reclosed Phase `0`
surface with no reclose event. Phase `4` reclosed on 2026-08-10 with Sprints `4.73`–`4.75`; Phase `1`
reclosed earlier on Sprint `1.83`. `prodbox dev check` / `dev docs check` / `dev lint docs` exit 0,
canonical `prodbox test unit` exits 0 at main Hspec **3294/3294**, and installed
`prodbox test integration cli` passes **55/55**, exit 0. **The AWS-substrate canonical suite does not
currently complete**: it is blocked at the broker on the defect Sprint `3.34` owns, which is a
sharper statement than the standing Standards O/P pending status and is recorded as such. Clean-room
deployment qualification remains pending on both substrates as the distinct Standards O/P axis.

**Beyond those two open sprints, the remaining plan work is not a sprint backlog but two named
axes**, and they must be worked in that order because the first invalidates the second:
...
1. The `Pending Removal` ledger (Exit Definition item 33). **Seventeen of its rows carried no owning
   ...
   owning sprint as a Standard-O recorder axis, none created. The rest are
   being worked in phase-numerical order as own-surface reopens (Standard A), which is why a closed
   phase reopening is the expected shape of progress here rather than a regression. **All six
   remaining unowned rows sit on Phase-`0`, `1`, and `5` surfaces**; Phases `1`, `3`, and `4` are
   reclosed.
   One of the eleven was closed by being **refuted** rather than fixed: the row asserting
   `prodbox test integration cli` fails 20 of 55 was measured against a baseline Sprint `5.31` had
   already repaired, and re-running the suite returns 55/55.
...
`3` is reclosed on Sprint `3.27`; the later numerical pass also reclosed Phases `4`–`8` on their
expanded owned surfaces, with Phase `5` most recently reclosed on `5.31`. Their exact closure
states are summarized in the Phase Overview and defined in the phase files; no later phase reopens
an earlier one. Production remains on the
...
control-plane roles as physically separate workloads. Sprint `2.35` unblocked the Planned live
serving validation in Sprint `5.22`. Deployment qualification remains pending on both substrates.
