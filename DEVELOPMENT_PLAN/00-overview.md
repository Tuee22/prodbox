...
| 2 | Gateway Runtime and DNS Ownership | 🔄 **Reopened on Sprint `2.134` (2026-09-11).** Sprint `6.5`'s investigation proved a long-lived thread that died silently because its spawn handle was discarded, and found five more under `src/` with the same shape — including the sole refresher of the cache `/readyz` serves. Chaos-hardening rule R6 already forbade discarding a join result; nothing enforced it. `2.134` repairs every site, lifts supervision into a type, and gates the rule repo-wide. Historical Phase-2 closure records remain in the sprint records. | Pure exact-manifest and plan-order proofs, stateful apply/read-back/idempotence/refusal tests, full local gate, then exact-image Job creation and attestation. |
...
| 4 | Lifecycle Hardening, Pulumi Decoupling, and Python Removal | 🔄 **Reopened on Sprints `4.92`–`4.94` (2026-09-11).** Fifteen of roughly eighteen codecs for `ObservationEvidenceScope` erase the DNS hosted zone Sprints `7.36`/`7.38` added and sealed; the drain's `kubectl` calls carry no wall clock at all; and the teardown interpreter's twenty result-kind mismatch arms exist only because operation and result are untied. Historical Phase-4 closure records remain in the sprint records. | Pure exhaustive registry-key/capability lookup, negative Provider-ID-alias proof, focused and full unit suites, documentation gates, and canonical `prodbox dev check`; live cascade recovery remains Standard-P evidence. |
| 5 | Canonical Test Suite | 🔄 **Reopened on Sprints `5.44`–`5.46` (2026-09-11).** Every assertion about the ephemeral client's bearer token was an absence assertion, no helper in `test/` measures elapsed time, three declared suites are compiled by the gate and run by no scope, and § 3.2's mandated codec round-trip properties do not exist. Historical Phase-5 closure records remain in the sprint records. | Exact fake-boundary regressions, installed CLI/env 63/63, full unit/auxiliary suites, docs/diff checks, and canonical `prodbox dev check`. |
...
| 7 | AWS Substrate Foundations | 🔄 **Reopened on Sprints `7.39`–`7.40` (2026-09-11).** The ephemeral Kubernetes client's bearer-token writer fails with `ENXIO` before `kubectl` starts and dies silently, so the client has never authenticated on any live run and no AWS teardown path has ever reached an EKS API server. A third statement of the machinery survives in `src/Prodbox/Infra/AwsEksTestStack.hs`, reachable from the legacy public cascade's AWS drain. Historical Phase-7 closure records remain in the sprint records. | Focused 4/4, unit 4589/4589 plus auxiliary components, installed CLI/environment 62/62 each, docs gates, and `dev check`; live Route 53 and current-revision deployment qualification remain pending. |
