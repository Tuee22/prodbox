...
-- | Transport-neutral 'AuthorityObjectCore' CAS/read/lease-guard
-- orchestration over an in-memory conditional-put fake. The fixture pins the
-- retained core's conditional-put semantics and fail-closed lease-guard ladder
-- without asserting any retired transport or physical object-store path.
module AuthorityObjectCoreCas
...
authorityObjectCoreCasSuite :: SuiteBuilder ()
authorityObjectCoreCasSuite =
  describe "AuthorityObjectCore transport-neutral CAS" $ do
    describe "read" $ do
