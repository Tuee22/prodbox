...
| Teardown | Current `prodbox cluster delete --yes` is local-only. Target `prodbox cluster delete --cascade --yes` (pending Sprints `4.85`, `4.86`, `5.36`, and `6.5`, consuming the completed Sprints `3.41`/`4.84` and the Sprint-`5.35` oracle) starts/resumes recover-to-clean. Private `ReadyToUninstallEvidence` admits uninstall only from exact clean evidence, a backed-up/read-back pre-uninstall report, and its one-shot permit; it is not completion. `CascadeCompleteEvidence` additionally requires exact `LocalUninstallEvidence` plus the matching read-back `LocalCompletionReceipt`. Incomplete runs return `CleanupRunId` plus `RecoveryPlaneDisposition` and claim a live plane only when the disposition is `Established`, never when `NotEstablished` or `Lost`. |
...
| Required Config | `route53.zone_id`, `domain.demo_fqdn`, the operator-authored ACME account email and typed EAB-generation references, `deployment.*`, and non-secret `ses.*` coordinates (sender_domain, receive_subdomain, capture_bucket — required for `keycloak-invite` validation). The ZeroSSL directory is one compiled provider-protocol identity, not a config coordinate. `config setup` remains Tier-0-only. ACME EAB material enters through its distinct schema-indexed external-material permit/ingress; it never reuses the AWS-admin session. Elevated/admin AWS power enters prodbox only through the interactive `SecretRef.Prompt`; under the harness the test-only `aws_admin_for_test_simulation.*` fixture in `test-secrets.dhall` (TestPlaintext, never imported by production config, never in Vault) simulates that prompt for the bounded identity plan and explicit admin actions. Missing any required coordinate or required material receipt fails fast; the home substrate does not fall back to AWS-substrate values. |
...
| Suite parity | Required teardown prerequisites: Sprints `4.85`, `4.86`, `5.36`, and `6.5`, consuming the completed Sprints `3.41`/`4.84` and the Sprint-`5.35` oracle. Sprint `8.12` then reruns `LCPC-2026-07-11`, `TEARDOWN-2026-08-15`, consecutive home aggregates, exact identities/envelopes/load, fault isolation, canonical restoration, invite flow, and residue re-observation as the sole final owner. Earlier runs remain revision-scoped historical evidence. Current status/evidence live only in [README.md → Deployment Qualification](README.md#deployment-qualification). |
...
| Field | Value |
|-------|-------|
| Provision | `prodbox aws stack eks reconcile` (EKS test cluster), `prodbox aws stack aws-subzone reconcile` (per-substrate Route 53 subzone), and `prodbox aws stack test reconcile` (three Ubuntu 24.04 EC2 instances for HA-RKE2). The three EC2 instances are the structural HA-RKE2 quorum. Desired presence for `eks` and `test` requires the validated `aws_substrate.profile`; its resource and network values travel inside the signed provider intent and through the existing Pulumi configuration transport. Positive `cluster_topology.Eks.node_group_size` supplies the EKS desired count, with the profile supplying its minimum and maximum bounds. Legacy provider-stack configurations remain readable for observation/destroy but cannot apply desired presence. |
| Teardown | Current explicit surfaces are `prodbox aws stack eks destroy --yes`, `prodbox aws stack aws-subzone destroy --yes`, and `prodbox aws stack test destroy --yes`. Sprint `7.36` implements exact independent desired-absence adapters for `aws-eks`, `aws-eks-subzone`, and `aws-test`; the qualification-gated public cutover makes each public destroy and cascade projection consume the same durable node and exact read-back. Existing pre-manifest stacks with unusable checkpoints enter only the bounded, plan-confirmed legacy-adoption protocol described below; discovery alone never authorizes mutation. |
...
| Required Config | `aws_substrate.subzone_name` (the AWS-substrate public FQDN, for example `aws.example.invalid`), optional `aws_substrate.hosted_zone_id` when an operator wants to pin the already-provisioned subzone ID in config, `ses.*` (sender_domain, receive_subdomain, capture_bucket — shared cross-substrate; same values as home substrate), AWS operator credentials, plus the same operator-authored ACME contact/EAB references the home substrate uses. During harness-driven AWS runs, the suite reads the live `aws-eks-subzone` Pulumi output after provisioning and passes the hosted-zone ID to child commands. Missing AWS-substrate values fail fast; the AWS substrate does not fall back to `route53.zone_id` or `domain.demo_fqdn` from the home substrate. `aws.region` is generated **empty** like every other operator coordinate, with no compiled default, so an unconfigured region refuses rather than resolving to a literal (Sprint `1.91`). `aws_substrate.profile` is optional only so a home-only deployment has no AWS envelope; every AWS desired-presence path requires it, and a present record has no optional members or compiled fallback. Its fields are `operator_cidr`, `eks_node_instance_type`, `eks_node_disk_size_gib`, `eks_node_min_size`, `eks_node_max_size`, three `aws_test_instance_types`, three `aws_test_root_volume_types`, three `aws_test_root_volume_sizes_gib`, `static_ebs_volume_type`, `eks_vpc_cidr`, two `eks_subnet_cidrs`, `aws_test_vpc_cidr`, and three `aws_test_subnet_cidrs`. The desired EKS size is the separate positive `cluster_topology.Eks.node_group_size` field. |
...
| Target cleanup | Sprint `7.36` implements the shared lifecycle graph adapters that observe each stack independently, drain EKS only through a positive-evidence expiring session, restore checkpoint backup or use a complete ownership manifest, reconcile exact controller/stack/EBS absence, then dispose Operational credentials and audit normalized escapees. For known pre-manifest VPC/EKS/Route 53/test-stack families, the provider-native read-only observer enumerates the complete registry-bounded family; an explicit admin permit over the exact rendered-plan digest may then receipt-commit and read back a `ConfirmedLegacyAdoptionManifest` before mutation. Missing, extra, ambiguous, partial, or unobservable candidates refuse. A retained bucket is never per-run inventory. Public-cascade activation remains qualification-gated and is not claimed here. |
...
| AWS | The home matrix plus two `prodbox test all --substrate aws` runs, all three exact stack destroy commands, and repeated cascade; independent observation of all three stacks; primary/backup/write-ahead-manifest/incomplete-manifest decision arms; the bounded plan-confirmed legacy-adoption success and every refusal arm for pre-manifest VPC/EKS/Route 53/test-stack families; provider-issued EKS drain-session binding; controller-family backstop; AWS unobservability refusal; retained bucket reported exactly once and never attributed to a per-run stack; **and a positive proof that the Kubernetes-reaching teardown authenticated** — an observation returned by the cluster, not the absence of an error, because every credential assertion on this path was until 2026-09-11 an absence assertion and the client had never authenticated (Sprint `7.39`) | [README.md → Deployment Qualification](README.md#deployment-qualification) |
...
| ALBs / NLBs / target groups / listeners / security groups | AWS Load Balancer Controller responding to a registered `Service`/`Ingress` owner | Sprint `7.36`: register account/region/cluster, deterministic manifest/name and tags; create the owner inert, CAS-enrich its Kubernetes-assigned UID, then enable controller mutation and CAS-enrich exact ARNs | Delete the Kubernetes owner while the controller is live, then use the registered bounded AWS-family discover/destroy/read-back program for any survivor. The tag query is a selector for that program and postflight evidence, not a diagnostic-only substitute. Public-route activation remains qualification-gated. |
...
| Recover-to-clean teardown and exact residue re-observation | **Open under Sprint `6.5`, consuming completed Sprints `3.41`, `4.84`–`4.86`, `5.35`, and `5.36`.** The validation harness is now a durable exact-keyed cleanup client; the remaining home work is the public single-writer cutover and qualification over bootstrap-owned recovery, honest `RecoveryPlaneDisposition`, private pre-uninstall readiness, distinct exact post-uninstall completion, and uninstall-last. | **Open under Sprint `6.5`, consuming the same completed common foundations plus Sprints `7.36` and `7.38`.** The exact stack/family observers, write-ahead and confirmed-legacy adoption manifests, typed EKS drain sessions, retained-bucket isolation, descriptor-bound validation client, and compiled DNS-zone scope are landed. `6.5` owns public cutover. Live all-three-stack proof remains non-blocking qualification evidence for `8.12`. |
