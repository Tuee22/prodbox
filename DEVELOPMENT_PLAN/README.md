...
| 2026-07-19 | Sprint `2.32` 🔄 **Active — single-writer emitter kernel (increment 1) landed** — the additive, code-owned first increment landed pre-cluster. `Prodbox.Gateway.Emitter.Kernel` is the pure decide/evolve transition machine: an explicit `stage → fsync → publish → commit → fsync` `EmitterPhase` whose single-writer property is *structural* (the whole in-flight transition is one `Maybe InFlight`), one `TransitionAdmission` + absolute `Deadline` + `Incarnation` minted at begin and threaded unchanged through every phase (no permit release/reacquire), park-the-**unsigned**-advance-then-rotate as **two separately-ticketed** transitions, incarnation fencing of stale-mount completions, idempotent crash-resume re-drive, and a **size-triggered** checkpoint fold so the unacked repair set stays bounded behind an unreachable peer. `Prodbox.Gateway.Emitter.Mailbox` is the bounded FIFO with heartbeat coalescing (ownership/epoch/recover never coalesced) and immediate overload rejection with a retry hint. `nextAnchorFor` is now exported from `Prodbox.Gateway.Continuity`, so the kernel delegates the monotonic sequence/epoch fence to it (`plannedSemanticAnchor`/`plannedEpochAnchor`) with no re-implementation. `test/unit/GatewayEmitterKernel.hs` (18 cases) proves single-writer ordering, monotonic advance, idempotent replay, crash-resume, stale-incarnation fencing, deadline propagation (pre-publish abort vs post-publish hold-for-re-drive), the park-and-resign rotation, mailbox bounds/coalescing, and the checkpoint. All four adversarial corrections from the verified design are encoded. Evidence: `prodbox dev check` exit 0, unit 1844/1844. The live Daemon cutover, the encrypted journal/fsync interpreter, the StatefulSet/EBS-`ReadWriteOncePod`/Lease-incarnation wiring, and the TLA model revision remain the non-blocking deferred cluster-adjacent Remaining Work. |
...
| 1 | Haskell Runtime, CLI, Config, and Pulumi Foundations | 🔄 **Reopened; Foundation Epoch Phase-1 sprints `1.63`–`1.66` ✅ Done.** Sprint `1.61` ✅ Done (capability algebra + graph lowering + runCapability interpreter + reconcile-driver cutover, live-validated 2026-07-18); `1.62` ✅ Done (absolute deadlines + serializable authority clock + `ServiceCapacityPlan`/bounded FIFO admission + native SigV4 IAM/STS/Route53/ServiceQuotas clients over a linear credential handle, validated pre-cluster 2026-07-18). | Operation-indexed capabilities, exact graph requirements, absolute deadlines, service-capacity algebra, native object-store and managed Vault-session boundaries, conformance tier and legacy escape registry, measured capacity certification |
| 2 | Haskell Gateway Runtime and DNS Ownership | 📋 **Reopened; Foundation Epoch Sprint `2.34` ✅ Done** (compiled service boundary — route registry + total dispatch + client/probe/wire-path projections — plus latched readiness projection and `GatewayChartStatics` all landed and validated pre-cluster). Sprint `2.32` is 🔄 Active (`1.62` Done — the pure emitter transition kernel + bounded mailbox + property suite (increment 1) landed + validated pre-cluster 2026-07-19; the live Daemon cutover + journal/fsync + StatefulSet/EBS/Lease + TLA revision are deferred cluster-adjacent Remaining Work); `2.33` by `2.32`. Sprint `2.35` is 🔄 Active — its pure `CertScope` algebra + property suite, the Tier-0 `cert_scopes` config field + fail-fast validation, the derived keycloak public-edge `dnsNames` (values-injection projection keyed on the served host), and the `edge status` certificate-expiry rungs all landed and validated pre-cluster; only the deliberately-deferred retention re-key (live-safety) remains code-owned, plus the live serving proof (Sprint `5.22`). | Single-writer emitter actor/journal, whole-transition ownership, Bootstrap Broker extraction, gateway scope reduction, compiled service boundary and latched readiness, configurable certificate-scope algebra and derived edge projections |
...
| 5 | Canonical Test Suite | 📋 **Reopened; Foundation Epoch Sprint `5.20` ✅ Done** (derived restore graph + total executor pure core AND the `TestRunner` live wiring both landed; end-to-end aggregate-report exercise is the non-blocking Standard-O axis). Sprint `5.21` is 🔄 Active (recorder gate landed; `1.65` Done, so no blocker — the live `--record-profile` collection + first committed profile remain the Standard-O axis). Sprints `5.18`–`5.19` follow `4.50`; `5.22` is blocked by `2.35`. | Capability-bound preparation, always-run cleanup DAG, CPU/queue/deadline/fault oracle, derived restore graph and total executor, measured-profile recorder, certificate-scope serving validation |
...
| 7 | AWS Substrate Foundations | 🔄 **Reopened; Foundation Epoch Sprint `7.34` Done** (harness postflight residue narrowed to per-run). Sprint `7.33` is blocked by `6.4`. | AWS Broker/Target-Agent/Gateway parity, exact client transport to the single retained home authority, resource isolation, prerequisite fault evidence, and per-run postflight residue narrowing |
