...
- **Phase `3` 🔄 Reopened** — Sprint `3.34` 📋 owns the coordinate and the chart-lint region. The
...
- **Phase `2` 🔄 Reopened** — Sprint `2.42` 📋 owns the broker's discarded transport failure. The
  chart caused the outage; this surface is why it cost eight runs to find, because `Left _` reduces a
  dropped packet, a `403`, and a `404` to one sentence while `/healthz` answers 200
