...
<!-- Prior Phase 2 status: 🔄 **Reopened on Sprint `2.109` (2026-08-28).** Sprint `2.108` is Done and live-proven: generation 73 renews the exact expired prepared state while retaining journal identity and reaches permit-bound J -->
<!-- Prior Phase 4 status: ✅ **Reclosed on Sprint `4.91` (2026-09-08).** Checkpoint custody now selects only the descriptor's canonical registered resource key and refuses the distinct Provider-facing Pulumi stack ID; exhaustiv -->
<!-- Prior Phase 5 status: ✅ **Reclosed on Sprint `5.43` (2026-08-31).** Sprints `5.38`–`5.43` make the fake Helm, Docker retention/image identity, Kubernetes stored read-back, and Authority Backup forwarding boundaries project -->
<!-- Prior Phase 7 status: ✅ Reclosed on Sprint `7.38` (2026-08-23). Sprint `7.36` completed the exact native stack, IAM/controller-family, manifest/adoption, EKS-drain, audit, and revocation adapters; `7.37` completed the type -->
...
| 2 | Gateway Runtime and DNS Ownership | ✅ **Reclosed on Sprint `2.134` (2026-09-11).** Sprint `6.5`'s investigation proved a long-lived thread that died silently because its spawn handle was discarded, and found five more under `src/` with the same shape — including the sole refresher of the cache `/readyz` serves. Chaos-hardening rule R6 already forbade discarding a join result; nothing enforced it. `2.134` converts every site onto `Prodbox.Supervision.withSupervisedChild`, whose private constructor makes an unlinked long-lived child unconstructible, and lands `checkSpawnedHandleDisposition` over every module under `src/` with no admission registry. The repo-wide rule found two further sites the sprint record did not enumerate — both statements of the ephemeral Kubernetes client's token writer — and they are converted too, so that proven-broken mechanism failed loudly at first execution rather than silently, until Sprints `7.39` and `7.40` replaced it and deleted the duplicate the same day. The Bootstrap Broker's request pool is repaired differently and the record says why: its handles are joined at drain, so the defect was *when* they were observed, and the manager now waits on the listener and the pool together. Historical Phase-2 closure records remain in the sprint records. | Pure exact-manifest and plan-order proofs, stateful apply/read-back/idempotence/refusal tests, full local gate, then exact-image Job creation and attestation. |
...
| 4 | Lifecycle Hardening, Pulumi Decoupling, and Python Removal | ✅ **Reclosed on Sprints `4.92`–`4.94` (2026-09-11).** Fifteen of roughly eighteen codecs for `ObservationEvidenceScope` erased the DNS hosted zone Sprints `7.36`/`7.38` added and sealed; the drain's `kubectl` calls carry no wall clock at all; and the teardown interpreter's twenty result-kind mismatch arms exist only because operation and result are untied. **All three are Done:** one canonical encoder in `Prodbox.Lifecycle.Teardown.ScopeCodec`, with every durable envelope nesting it and every projection derived from one field set a new scope field cannot be added to without a compile error at each; every drain `kubectl` bounded under a wall clock derived from the per-request bound it now always carries, with a bounded refusal reported as unobservable and the completion poll's budget turned into an absolute deadline; and `TeardownOperation` indexed by result as well as surface, which deletes the twenty catch-all arms and the one string they collapsed into by making the state they guarded unrepresentable. Historical Phase-4 closure records remain in the sprint records. | Pure exhaustive registry-key/capability lookup, negative Provider-ID-alias proof, focused and full unit suites, documentation gates, and canonical `prodbox dev check`; live cascade recovery remains Standard-P evidence. |
| 5 | Canonical Test Suite | ✅ **Reclosed on Sprints `5.44`–`5.46` (2026-09-11).** Every assertion about the ephemeral client's bearer token was an absence assertion, no helper in `test/` measured elapsed time, three declared suites are compiled by the gate and run by no scope, and § 3.2's mandated codec round-trip properties do not exist. **All three are Done:** a real-child harness that asserts a credential arrived, proven able to disagree with its subject, plus monotonic elapsed-time vocabulary and a negative control pinning the wedge signature; every declared suite routed into a `prodbox test` scope behind a declared-versus-routed bijection gate, which on first execution exposed a style assertion that was false about the architecture and a daemon suite failing 23 of its 27 cases; and the codec round-trip properties § 3.2 mandated, over the canonical scope codec, with the mutation claim tested rather than asserted. Historical Phase-5 closure records remain in the sprint records. | Exact fake-boundary regressions, installed CLI/env 63/63, full unit/auxiliary suites, docs/diff checks, and canonical `prodbox dev check`. |
| 6 | Final Clean-Room Rerun and Handoff | 🔄 Sprint `6.5` is Active. Sprint `4.91` closed its checkpoint-custody dependency; deploy that corrected runtime, recover the registered AWS graph through the supported cascade, restore the mandatory retained control plane, then resume the public generic/home sole-writer qualification. | Pure cutover/resume/rollback folds, installed fake traces, repository absence guards, unit/integration gates, then current-revision qualification under Standard P. |
| 7 | AWS Substrate Foundations | ✅ **Reclosed on Sprints `7.39` and `7.40` (2026-09-11).** The ephemeral Kubernetes client's bearer-token writer failed with `ENXIO` before `kubectl` started and died silently, so the client had never authenticated on any live run and no AWS teardown path had ever reached an EKS API server; a third statement of the machinery survived in `src/Prodbox/Infra/AwsEksTestStack.hs`, reachable from the legacy public cascade's AWS drain. `7.39` replaced the rendezvous with a private file written under `O_EXCL`, `O_NOFOLLOW`, `CLOEXEC` and owner-only mode and read back before the client value exists, chosen on a measurement rather than on the design argument it displaced: `kubectl` opens its token file exactly twice per invocation and independently of request count, so no stream served once can satisfy it, and the rendezvous is retired as a class. `7.40` routed `withEksKubeconfig` onto the one owning module, deleted the duplicate renderer, pipe, writer and orphaned imports, and landed `checkEphemeralCredentialMachinery`, because the singular claim had already been recorded in four documents and survived being false. Historical Phase-7 closure records remain in the sprint records. | Gate cases 5/5 and credential cases 5/5, both exercised against the real tree as well as synthetically, primary unit 4972/4972, and canonical `dev check` 0. **Live 2026-09-12:** cascade-qualification cycle `recovery-ephemeral-credential-file` reached and passed the drain's Kubernetes UID observation — the first EKS API authentication on any live run — so both sprints' live halves are proven; live Route 53 and current-revision deployment qualification remain pending and non-blocking under Standards O/P. |
...
| Ordinary teardown recovery profile | Minimal RKE2/API, retained MinIO/Vault bindings, Broker unseal path, bootstrap-core external caller identity, Lifecycle Authority, Backup Adapter, Provider Worker, and conditionally required Target Agent | Derived capability closure; excludes Gateway/apps and resumes old cleanup before new admission. Its canonical durable identity retains the descriptor's optional hosted-zone scope and reconstructs that exact scope at final disposition; retained pre-zone identities remain zoneless. An incomplete result distinguishes established, never-established, and subsequently lost recovery planes. |
