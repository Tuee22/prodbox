...
**Current head state (2026-08-02 — all code-owned phases `0`–`8` are closed; deployment
qualification remains pending on both substrates).** Sprint `8.12` closes the non-partial
...
| 7 | AWS Substrate Foundations | ✅ **Reclosed on Sprint `7.33` (2026-08-02).** Role/transport isolation, target-local DNS01, deterministic IAM/controller ownership, exact public-A Provider intents, and fault dispositions are code-locally validated; live AWS evidence remains Standard O/P. | AWS Broker/Target-Agent/Gateway parity, exact client transport to the single retained home authority, resource isolation, prerequisite fault evidence, and per-run postflight residue narrowing |
