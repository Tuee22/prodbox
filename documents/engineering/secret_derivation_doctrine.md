...
| TLS-retention-store AWS identity | `secret/aws/tls-retention-store` (KV) | `policy/aws-tls-retention` | separately deployed TLS Retention Adapter only; `LongLived`, ciphertext-only, and restricted to exact `public-edge-tls/<substrate>/<fqdn>` prefixes |
