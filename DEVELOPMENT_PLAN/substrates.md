...
| Suite parity | 🔄 Provisioning + SSH reachability only. The AWS substrate does not yet stand up the canonical chart set, cert-manager, ingress, Keycloak, or the public-edge stack; it does not yet run `charts-vscode`, `charts-api`, `charts-websocket`, or the public-edge proofs. Parity sprint is tracked in [phase-7-aws-substrate-foundations.md](phase-7-aws-substrate-foundations.md). |
