...
**2026-07-02 — Unified block storage across substrates scheduled (documentation + plan only):
static pre-created EBS as `Retain` PVs on EKS.** An abnormal AWS bill traced to leaked, unattached
`gp2` EBS volumes from EKS test runs (the dynamic `gp2` path with `reclaimPolicy: Delete` orphaned
CSI-provisioned volumes on teardown) motivates unifying block storage onto the home substrate's
static model. The SSoT
...
`tiered_storage_capacity_doctrine.md` are reconciled to defer to the SSoT. **No new phase**
(Standard E preserved): three phases reopen to expand their own owned surface — **Phase 7** (Sprint
`7.28` static EBS PVs superseding the dynamic `gp2` path of `7.5.c.i`; Sprint `7.29` EKS VPC
ownership hardening + always-fresh test VPC), **Phase 4** (Sprint `4.39` EBS as a registered managed
resource with retain/test-scoped tag markers; Sprint `4.40` suite postflight test-EBS reaper +
retain-safe drain), and **Phase 5** (Sprint `5.12` the `eks-volume-rebind` identical-rebinding
validation — substrate-agnostic suite content, AWS run a non-blocking parity axis). Each reopen
