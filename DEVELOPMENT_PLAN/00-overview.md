...
|-----------|-----------|----------|------------------|
| Home local | `prodbox cluster reconcile` + `prodbox charts reconcile ...` | Current local-only `prodbox cluster delete --yes` preserves LongLived roots and makes no AWS claim. Target `prodbox cluster delete --cascade --yes` is pending Sprints `5.36` and `6.5`, consuming the completed Sprint-`3.41` recovery renderer, Sprint-`5.35` oracle, the Sprint-`4.86` candidate entrypoint that closed 2026-08-20, and the Sprint-`4.89` custodial-capability disposition that closed 2026-08-21. Sprint `4.88` closed the same day: the current cascade no longer takes the local-only no-install short-circuit, so a cascade that reaches no phase exits non-zero, and no cascade exit narrates permission to retire the retained root. | Current parity and deployment qualification live only in [README.md → Substrate Parity](README.md#substrate-parity) and [Deployment Qualification](README.md#deployment-qualification). |
...
| 7 | AWS Substrate Foundations | ✅ Reclosed on Sprint `7.33` (2026-08-02); AWS role/transport isolation, deterministic IAM names, the pure controller-transition algebra, target DNS01, public-A Provider intents, and fault dispositions were code-locally complete. Production owner-UID/child-ARN registration and provider-family cleanup were not wired; Sprint `7.36` owns that later correction. **Own-surface reopen, 2026-08-22 (Standard A/N)**: Sprint `7.36` gave the registered-target interpreter a Kubernetes-scoped execution arm and registered the DNS01 challenge record family behind it — the first registered family whose observation and read-back are Provider reads while its removal is a cert-manager owner delete, because the solver would rewrite a record the Provider deleted underneath it. Registering it exposed that the compiled program's observation scope has no producer for the run's DNS hosted zone, which is **new Sprint `7.38`**; Sprint `6.5` declares the backward dependency, and nothing live is affected because `runNativeDeleteCascade` is still the public writer and does not use the compiled program. | AWS topology rendering/fakes followed by the current-revision AWS isolation and cleanup campaign. |
