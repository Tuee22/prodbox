...
**Current head state (2026-08-13, third entry — Phase `3` reopens and recloses on Sprints `3.36` and
...
**Every code-owned phase surface is closed (2026-08-13).** Phase `4` recloses on Sprints `4.78`,
...
| 3 | `prodbox vault init` fail-closes: the Bootstrap Broker refuses the secret-bearing physical call as having *"bypassed its attested one-shot worker"*, and **no SecretWorker Job is ever created** | **Open** — see below |
...
**All nine phases are closed on their code-owned surfaces, and no phase has an open sprint
(2026-08-13).** Phase `1` reclosed most recently on Sprint `1.89`, which closed the
...
1. The `Pending Removal` ledger (Exit Definition item 33). **Seventeen of its rows carried no owning
   ...
   **The pending total is now derived rather than stated**, and the derivation is recorded in
   [legacy-tracking-for-deletion.md](legacy-tracking-for-deletion.md): **66** rows, of which **4**
   carry no owning sprint — two recorded by Sprint `1.89` on the day it closed the previous sole
   survivor, one by Sprint `3.37`, and **two opened by the first Standard-P campaign attempt itself**.
   Those last three are the first rows this ledger has ever gained from *running* the composed system
   rather than reading it. The figure this paragraph previously carried (`73`) was a restatement and
   was wrong by five; correcting it is exactly the move Sprints `0.27` and `0.28` made for their own
   inventories.
   The rows are worked in phase-numerical order as own-surface reopens (Standard A), which is why a
   closed phase reopening is the expected shape of progress. **Every phase is currently reclosed with
   no open sprint.** The three remaining unowned rows are two Sprint `1.89` residuals on Phase `1`
   and Phase `7` surfaces — `awsDefaultRegion`'s silent `us-east-1` substitution, and the provisioning
   functions that take a bare `ConfigFile` and validate it internally — plus Sprint `3.37`'s:
   cert-manager is the only mirrored platform component with no fallback source, which is what turned
   one broken upstream artifact into a forced component-image identity change.
