            ...
            if continue
                then do
                    ...
                    case nextMessageCountResult of
                        Left _ -> pure ()
                        Right nextCount ->
                            runWebsocketConnectionLoop runtime redisSocket clientSocket token sessionId nextCount closeVar frameBuffer
                else pure ()
...
handleOidcCallback runtime clientSocket request queryParams =
    case (Map.lookup "code" queryParams, Map.lookup "state" queryParams, lookupCookie oidcStateCookieName request) of
                ...
                case exchangeResult of
                    ...
                    Right tokenResponse ->
                        case buildOidcSessionFromTokens tokenResponse of
                            Left err -> sendPlainTextResponse clientSocket 500 [] err
                            Right session -> do
                                storeResult <- withRedisSocket (websocketRuntimeRedisConfig runtime) (\redisSocket -> storeOidcSession redisSocket session)
                                case storeResult of
                                    Left err -> sendPlainTextResponse clientSocket 500 [] err
                                    Right () ->
                                        sendPlainTextResponse
...
registerWebsocketConnection :: WebsocketRuntime -> IO (Either String (Int, TVar (Maybe ConnectionCloseReason)))
...
prepareWebsocketSession redisSocket podName sessionId resetRequested = do
    whenReset <- if resetRequested then redisDel redisSocket (messagesKey sessionId) else pure (Right ())
    case whenReset of
...
publishWebsocketMessage :: Socket -> AuthorizedToken -> String -> String -> String -> IO (Either String Int)
                    ...
                    case connectedByResult of
                        ...
                        Right () ->
                            redisRpush
                                ...
                                (renderWebsocketMessage WebsocketMessage{websocketMessageSenderPod = podName, websocketMessagePayload = messageBody})
