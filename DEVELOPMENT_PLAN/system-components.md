...
The inventory documents the authoritative current Haskell-only implementation, including the
public-edge expansion that still needs aggregate validation closure before handoff. When the cleanup
ledger is non-empty,
...
| Chart platform and retained state | Haskell chart registry, PostgreSQL platform constants, retained-storage reconciler, CLI runtime, the Percona-operator-backed application-database contract, the supported auth-model split between Envoy-authenticated browser delivery for proxy-auth workloads and app-managed OIDC workloads on the dedicated Keycloak host, the explicit bearer-token plus browser cookie or session auth-carrier doctrine, JWT-only API delivery from Keycloak JWKS-backed Envoy validation, and the Redis-backed WebSocket-host chart surface with workload-managed OIDC bootstrap, real `/ws` upgrade, and any later explicit external rate-limit-service surface | `src/Prodbox/CLI/Charts.hs`, `src/Prodbox/Lib/ChartPlatform.hs`, `src/Prodbox/Lib/Storage.hs`, `src/Prodbox/PostgresPlatform.hs`, `charts/`, plus generated retained non-PV state under `.prodbox-state/` |
| Public workload runtime | Haskell internal runtime for the supported API workload plus the WebSocket workload selected through environment rather than repo-root config; the current runtime owns workload-managed OIDC bootstrap, real `/ws` upgrade, Redis-backed reconnect-safe state, one-connection-per-pod lifetime, and readiness-based drain | `src/Prodbox/Workload.hs` |
...
| Public edge controller | Envoy Gateway plus Kubernetes Gateway API, with browser-facing auth enforced through `SecurityPolicy` for proxy-auth workloads, request-carried API bearer tokens validated locally at Envoy from Keycloak issuer metadata plus JWKS-backed signing keys and route claims, dedicated-host Keycloak integration, public TLS termination at Envoy, and WebSocket-capable proxying on the same public edge | RKE2 workload | Haskell lifecycle plus closed public-host doctrine | Cluster resources |
...
| Namespace-local identity stack | `keycloak` on the dedicated public identity hostname, with the supported external-hostname, issuer, proxy-header, and public-path contract for chart-managed workloads; new login, refresh, and later JWKS refresh depend on this workload while the steady-state JWT hot path remains local to Envoy | RKE2 workload | Haskell chart platform plus external Percona-operator-backed Patroni PostgreSQL dependency | Cluster resources plus retained roots |
| Namespace-local app stack | `vscode` behind an Envoy-authenticated dedicated public app route, plus the `api` workload and the direct-OIDC `websocket` workload on their own dedicated public hosts | RKE2 workload | Haskell chart platform | Cluster resources plus retained roots |
...
| Public-edge diagnostic | `prodbox host public-edge` | Classify Route 53, Gateway API, TLS, dedicated identity/browser/API/WebSocket host routing, advertisement mode, and external-reachability state |
...
| Public workload runtime | `prodbox workload start` | Run the internal API runtime or the direct-OIDC WebSocket runtime selected through `PRODBOX_WORKLOAD_MODE=api|websocket` for the chart-managed public workloads, including the real `/ws` upgrade, one-connection-per-pod lifetime, and readiness-based drain contract |
| Chart runtime | `prodbox charts list|status <chart>|deploy <chart>|delete <chart> [--yes]` | Manage the chart platform, including Keycloak on the identity route, `vscode` on the Envoy-protected browser route, the supported auth-model split for direct-OIDC workloads, and the API, Redis, and WebSocket workload surfaces |
...
| Chart platform | `prodbox test integration charts-storage`, `prodbox test integration charts-platform`, `prodbox test integration charts-vscode`, `prodbox test integration charts-api`, and `prodbox test integration charts-websocket`, plus manifest and lifecycle proof that supported Patroni use flows through the Percona operator-backed platform, Keycloak-backed workloads preserve the dedicated identity-host and proxy-aware issuer contract, JWT API auth is enforced at Envoy from explicit bearer-token and JWKS configuration, and the WebSocket host proves direct-OIDC bootstrap, real WebSocket upgrade behavior, Redis-backed state continuity, revocation-driven reconnect, and readiness-based drain |
| Public-host proof | `prodbox host public-edge`, `prodbox test integration public-dns`, `prodbox test integration charts-api`, and `prodbox test integration charts-websocket` on the dedicated-hostname Gateway API target doctrine, including Keycloak identity-host redirect and issuer proof, workload-owned direct-OIDC session proof, real WebSocket upgrade behavior, one-connection-per-pod lifetime, revocation-driven reconnect, and readiness-based drain |
