...
| 4 | Lifecycle Hardening, Pulumi Decoupling, and Python Removal | ✅ **Reclosed on Sprint `4.91` (2026-09-08).** Checkpoint custody now selects only the descriptor's canonical registered resource key and refuses the distinct Provider-facing Pulumi stack ID; exhaustive coverage includes the load-bearing `aws-eks` / `aws-eks-test` unequal-name case. Focused 13/13, primary 4,891/4,891, auxiliaries 27/35/36, documentation/diff gates, and canonical `prodbox dev check` pass. Live recovery remains Phase-6 Standard-P evidence. Prior: ✅ Reclosed on Sprint `4.90` (2026-08-23), with earlier closure history retained in the phase file. | Pure exhaustive registry-key/capability lookup, negative Provider-ID-alias proof, focused and full unit suites, documentation gates, and canonical `prodbox dev check`; live cascade recovery remains Standard-P evidence. |
