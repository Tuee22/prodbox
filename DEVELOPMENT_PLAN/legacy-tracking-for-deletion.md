...
| Cross-arch `docker buildx --platform linux/amd64,linux/arm64 --push` custom-image publication and mixed-arch container assumptions in lifecycle code, tests, and doctrine docs | Sprint `4.1` | The supported lifecycle now uses only native-host-architecture Docker builds: `amd64` hosts publish `amd64` images, `arm64` hosts publish `arm64` images, Apple Silicon with Colima is valid on the native `arm64` path, and no supported path uses `docker buildx`, cross-arch emulation, or mixed-arch cluster closure. |
...
| Arch-implicit container population doctrine without explicit architecture-handling rules | Sprint 4.1 closure on April 18, 2026 | That early closure made image-architecture handling explicit, but the later cross-arch `docker buildx` and mixed-arch doctrine is now reopened separately in `Pending Removal` because the supported lifecycle has shifted to native-host-architecture builds only. |
