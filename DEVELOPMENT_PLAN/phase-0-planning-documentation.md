...
## Sprint 0.30: A Dependency The Gate Cannot Read Is A Dependency Nobody Enforces ✅

**Status**: ✅ **Done (2026-08-18)** — Phase `0` own-surface reopen (Standard A) on the
plan-document metadata surface this phase already owns through Sprints `0.21` and `0.27`. It neither
recloses nor reopens the phase on any other account, and it changes no production composition.
**Implementation**: `src/Prodbox/CheckCode.hs` (`compareSprintIds`, `sprintDependencyFields`,
`sprintLiveDependencyIds`, `sprintDependencyDirectionViolations`,
`pendingRemovalPrerequisiteViolations`, `checkPlanDependencyDirection`, wired into
`runGovernedDocChecks`), `test/unit/Main.hs`, and the re-scoped plan surfaces
`DEVELOPMENT_PLAN/development_plan_standards.md`,
`DEVELOPMENT_PLAN/phase-4-lifecycle-canonical-paths.md`,
`DEVELOPMENT_PLAN/phase-6-clean-room-handoff.md`,
`DEVELOPMENT_PLAN/phase-7-aws-substrate-foundations.md`, and
`DEVELOPMENT_PLAN/legacy-tracking-for-deletion.md`.
**Blocked by**: none.
**Deployment qualification**: pending (not invalidated) — this sprint changes plan documentation and
a developer-tooling gate; it moves no production-composition surface. `SourceIdentity` binds governed
documentation, so these edits do move the source-manifest digest a future `proven` row would bind.
**Independent Validation**: `prodbox dev lint docs` and `prodbox dev check` exit 0 on the corrected
plan, four mutation exercises that each reintroduce one shape of the defect and confirm the gate
names the offending sprint or row, and pure unit cases over `compareSprintIds`,
...
Remove every backward dependency from the plan, and make the direction rule mechanical so the class
cannot silently return. Sprint `0.27` is the shape: measure, correct from evidence, then make the
requirement a gate.

### The 2026-08-17 audit named this blind spot and did not close it

That audit found a queue in which **no row could reach `Done`** — `4.84` held work it said could only
land inside `4.85`/`5.36`/`6.5`, `4.85` was `Parked` behind it, and `6.5` was `Blocked` on both — and
it recorded exactly why nothing had caught it: the dependencies "lived in prose rather than in a
`**Blocked by**` field, [so] `prodbox dev lint docs` could not see them and reported the queue clean."

The remedy was applied to the rows it found. The blind spot was left open, and by 2026-08-18 the same
class had re-accumulated in Sprint `4.85`:

- the `ExplicitLongLived` completion minter, which `registeredTargetExecutorViolations` makes one
  change with Sprint `7.36`'s retained-EBS adapter — "cannot land in either order without the other";
- the DNS01 Challenge/TXT ledger row, whose own constraint measurement says the executor "has to land
  with the registration", over a Route 53 path that is `7.36`'s;
- the terminal cascade audit ledger row, which states verbatim that "Sprint `7.36` supplies the
  Provider adapter";
- the callback-cleanup-client row, which `4.85` disclaims and `5.36` had already received — leaving
  the ledger as the one surface still naming a `Done`-bound `4.85` as its remover;
- validation item 10, whose "bidirectional parity across the complete closed universe" reading
  requires the compiled program and the signed manifest to converge, which is `6.5`'s cutover over
  `7.36`'s adapters.

A sixth had no owner at all: wiring `residueLayerAnswersResourceExistence` into
`residueBlocksTeardownGate` existed only as a paragraph inside another row, so `5.36` could have
closed that row and left the predicate permanently unwired.

### What the gate reads, and what it deliberately does not

`**Blocked by**` and `**Closure dependency**` are now the only fields in which a dependency may be
recorded, and `sprintDependencyDirectionViolations` fails the build on a live entry naming a later
sprint. Three Active sprints had been declaring theirs under `**Closure dependencies**` /
`**Closure dependency**` — the first a field Standard H did not define — so an unrecognized
dependency-shaped field name is itself a violation rather than a silent bypass.

Two decisions keep the gate honest rather than merely loud:

- **Historical lines record their own resolution in place** — a struck `Sprint 2.19` annotated
  `unblocked`, a `Sprint 1.38` annotated `(closed)`, a `Sprint 0.8` annotated `resolved`. Those are
  statements that a dependency
  is gone. `sprintLiveDependencyIds` drops struck spans and resolution annotations, so the ~130
  existing `**Blocked by**` lines keep working and the gate reports only live dependencies.
- **Prose is not scanned.** A prose scan would have to guess whether a sprint mention is a
  dependency or a disclaimer, and disclaiming later work is the *correct* pattern the plan uses
  everywhere. Instead the direction is made declarable: Standard N now says prose may only disclaim,
  and Standard I gives the ledger a `**Prerequisite**` field so a row's dependency is data.
  `pendingRemovalPrerequisiteViolations` fails when a row's declared prerequisite is later than its
  owner — the precise shape of "a row nobody in the queue can close".

`compareSprintIds` orders by phase number, then numerically per segment, so `7.10` sorts after `7.5`
and `7.5.b.i` after `7.5.b`. String comparison would have inverted both.

### The correction is ownership, not deletion

Every moved item lands on the sprint that owns the capability it waits on, using the
`**Received from Sprint X**` bullet pattern Sprint `5.36` already carried twice, and Sprint `4.85`
gains a disclaimer for each. This is the same remedy the `dns-aws` registration took on 2026-08-17,
when a Phase-4 registration whose program could not complete until a Phase-7 adapter landed was
withdrawn and re-owned to `7.36`.

Sprint `4.85` **stayed `Active`**. Its surviving work — the eight
`OperationalCredentialDispositionBlocker`s — was type-level and program-level absence on this phase's
own lifecycle-core surface, so it was same-phase work. What changed is that `4.85` became closable by
doing its own work, which is the whole point; it closed on 2026-08-18 with all eight retired and
measured.

### Validation

1. **Direction.** `sprintDependencyDirectionViolations` over the nine phase documents reports no
   violation, and the ~130 existing `**Blocked by**` lines — including every struck and
   resolution-annotated historical form — produce none.
2. **Mutation exercises.** Each reintroduces one shape of the defect and must be named by the gate,
   then reverted: a **Blocked by** line on a Phase-4 sprint naming Sprint `7.36`; the same line with
   that id struck through and annotated `unblocked` (which must *not* fire); a dependency field
   renamed back to `**Closure dependencies**`; and a `4.85`-owned `Pending Removal` row given a
   **Prerequisite** field naming Sprint `7.36`.
3. **Ordering.** Pure cases prove `compareSprintIds` puts `7.5` before `7.10`, `7.5.b` before
   `7.5.b.i`, and `4.85` before `5.36` before `6.5` before `7.36`.
4. **Ledger.** `pendingRemovalOwnerViolations` still passes, every moved row names a live queue
   sprint, and the two new rows carry an owner and a prerequisite that satisfies the direction rule.
5. `prodbox dev lint docs`, `prodbox dev docs check`, `prodbox dev check`, and `prodbox test unit`
   all exit 0.

### Remaining Work

None on this sprint's surface. One adjacent finding is recorded rather than fixed: **46 of the 77
`Pending Removal` rows name a sprint whose block is `Done`**, so their removal is unscheduled.
Standard I permits a `Done` replacement beside a surviving helper and Standard C requires no
remaining sprint-owned work, and the two have never been reconciled for the `Owning Sprint` column.
Folding a 46-row re-ownership sweep into a Standard-N direction fix would have made both harder to
review, so it is an explicitly `Unowned` ledger row.
