...
| Chart platform and retained state | Haskell chart registry, PostgreSQL platform constants, retained-storage reconciler, CLI runtime, the Percona-operator-backed application-database contract, Envoy-authenticated browser delivery, JWT-only API delivery, and Redis-backed WebSocket delivery | `src/Prodbox/CLI/Charts.hs`, `src/Prodbox/Lib/ChartPlatform.hs`, `src/Prodbox/Lib/Storage.hs`, `src/Prodbox/PostgresPlatform.hs`, `src/Prodbox/Workload.hs`, `charts/`, plus generated retained non-PV state under `.prodbox-state/` |
...
| Public edge controller | Envoy Gateway plus Kubernetes Gateway API, with browser-facing auth enforced through `SecurityPolicy`, JWT-only API validation and route claims enforced locally at Envoy, and WebSocket proxying on the same dedicated-host edge | RKE2 workload | Haskell lifecycle plus closed public-host doctrine | Cluster resources |
...
| Namespace-local app stack | `vscode` behind an Envoy-authenticated dedicated public app route, plus `api` and `websocket` workloads on their own dedicated public hosts | RKE2 workload | Haskell chart platform | Cluster resources plus retained roots |
| Shared realtime state | Redis backing supported WebSocket or rate-limit workloads and never Envoy JWT validation | RKE2 workload | Haskell chart platform plus public workload doctrine | Cluster resources plus app state |
...
| Chart runtime | `prodbox charts list|status <chart>|deploy <chart>|delete <chart> [--yes]` | Manage the chart platform, including Keycloak on the identity route, `vscode` on the Envoy-protected browser route, and the API, Redis, and WebSocket workload surfaces |
...
| Chart platform | `prodbox test integration charts-storage`, `prodbox test integration charts-platform`, `prodbox test integration charts-vscode`, `prodbox test integration charts-api`, and `prodbox test integration charts-websocket`, plus manifest and lifecycle proof that supported Patroni use flows through the Percona operator-backed platform, JWT API auth is enforced at Envoy, and Redis-backed WebSocket delivery remains reconnect-safe |
| Public-host proof | `prodbox host public-edge`, `prodbox test integration public-dns`, `prodbox test integration charts-api`, and `prodbox test integration charts-websocket` on the dedicated-hostname Gateway API target doctrine |
