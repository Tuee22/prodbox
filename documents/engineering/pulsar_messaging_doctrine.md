...
> **Implementation status.** This capability is scheduled, not built. The gateway/Orders and
> at-least-once envelopes move to CBOR in Phase 2 Sprints `2.27`–`2.28`; the self-maintained
> native Pulsar client and its chart land in Phase 3 Sprint `3.21`. Per
...
The gateway already carries the exact envelopes this doctrine governs. Today
[`src/Prodbox/Gateway/Types.hs`](../../src/Prodbox/Gateway/Types.hs) encodes `Orders`,
`SignedEvent`, and the anti-entropy `CommitLog` with `Data.Aeson` (JSON), and `SignedEvent` carries
a `payloadJson :: String` field:

```haskell
-- File: src/Prodbox/Gateway/Types.hs  (current surface — JSON, migrating to CBOR in Sprint 2.27)
...
Under this doctrine, Sprint `2.27` re-serializes the gateway gossip transport and the `Orders`
document to canonical CBOR (adding the `cborg` dependency), and Sprint `2.28` moves the durable
at-least-once event store (`src/Prodbox/Daemon/Events.hs`) to CBOR. The `payloadJson` field becomes
a `CborPayload`; the hash-chain and dedup guarantees are preserved precisely because the encoding is
canonical (§2.2). This is the concrete removal of the superseded "protobuf" language: neither JSON
nor protobuf is a sanctioned prodbox wire format after these sprints — CBOR is.
...
prodbox owns a single native-protocol Haskell Pulsar client — no WebSocket gateway, no second
language runtime — the single-node realization of the amoebius umbrella
...
client is unconditional: every payload it produces or consumes is a `CborPayload` (§2). The client
and its Pulsar chart land in Sprint `3.21`.
...
- **Linked dependents** (the modules that will implement this doctrine):
  `src/Prodbox/Gateway/Types.hs` (Orders / SignedEvent / CommitLog → CBOR, Sprint `2.27`),
  `src/Prodbox/Gateway/Peer.hs` (anti-entropy gossip transport → CBOR, Sprint `2.27`),
  `src/Prodbox/Daemon/Events.hs` (durable at-least-once store → CBOR, Sprint `2.28`), and the
  scheduled `src/Prodbox/Pulsar/Codec.hs`, `src/Prodbox/Pulsar/Topic.hs`,
  `src/Prodbox/Pulsar/Envelope.hs`, and `src/Prodbox/Pulsar/Client.hs` (native client + `Work*`
  family, Sprint `3.21`).
