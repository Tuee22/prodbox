...
scheduled under Sprints 3.18 / 3.19.)
...
is scheduled under Sprint 3.18.)
...
Kubernetes auth and materialized into k8s Secrets per
[Secret Derivation Doctrine](./secret_derivation_doctrine.md) §6:

| Secret (logical key) | Rendered k8s Secret | Source |
|---|---|---|
| `keycloak_admin_password` | `keycloak-runtime.admin` | Vault KV, fetched via Vault k8s auth |
| `patroni_app_password` | `prodbox-<root-chart>-pg-pguser-keycloak` | Vault KV, fetched via Vault k8s auth |
| `patroni_superuser_password` | `prodbox-<root-chart>-pg-pguser-postgres` | Vault KV, fetched via Vault k8s auth |
| `patroni_standby_password` | `prodbox-<root-chart>-pg-primaryuser` | Vault KV, fetched via Vault k8s auth |
| `keycloak_vscode_client_secret` | `keycloak-runtime.vscodeClient` | Vault KV, fetched via Vault k8s auth |
| `keycloak_api_client_secret` | `keycloak-runtime.apiClient` | Vault KV, fetched via Vault k8s auth |
| `keycloak_websocket_client_secret` | `keycloak-runtime.websocketClient` | Vault KV, fetched via Vault k8s auth |
| `keycloak_demo_user_password` | `keycloak-runtime.demoUser` | Vault KV, fetched via Vault k8s auth |
...
(Vault-KV chart-secret consumption is scheduled under Sprints 3.18 / 3.19.)
...
- **Vault is a singleton platform component.** Vault stands up on the same footing as
  ...
  platform component is scheduled under Sprint 3.17. See
...
- **Chart workloads consume Vault-held secrets via Vault Kubernetes auth only.** Chart workloads —
  ...
  fields are typed `SecretRef.Vault` references resolved against Vault at runtime. This Vault
  Kubernetes auth path for chart and Keycloak secrets is scheduled under Sprints 3.18 / 3.19. See
...
- **A sealed Vault fails secret-dependent startup closed.** When Vault is sealed, Keycloak
  ...
  from any non-Vault source — there is no plaintext fallback. This sealed-state behavior is
  scheduled under Sprint 3.18. See
