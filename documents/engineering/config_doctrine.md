...
- **TIER 0 — NON-SECRET BINARY CONTEXT.** A binary-owned, GENERATED, project-local `prodbox.dhall`
  ...
  binary (`writeTier0` / `ensureBasicsFloor`), not hand-authored — by `prodbox config setup` on the
  operator host, or by the test harness for tests. There is **NO fallback default**: an operation
  that needs `prodbox.dhall` (e.g. the `vault init` Tier-0 floor stamp,
  `writeTier0FloorPreservingParameters`) **fails fast** when it is absent rather than synthesizing a
  default — only `config setup` (which authors it) and the baked container default may seed from
  `defaultProjectConfig`. IN-CLUSTER: the built prodbox
  CONTAINER ships a default `prodbox.dhall` (`docker/default-prodbox.dhall`, GENERATED at
  image-build time from `renderDaemonContainerDefaultDhall`); the cluster daemon OVERWRITES it from
  a ConfigMap (hostbootstrap's per-frame context-init pattern). NOTE: the ConfigMap-overwrite
  already exists today (`gateway-config-<nodeId>` mounted at `/etc/gateway/config`, directory-mount
  so kubelet's atomic `..data` swap fires the fsnotify reload); the remaining gap is the
  in-container DEFAULT `prodbox.dhall` + the binary-context shape + the rename (Sprint `1.41`).
...
- `prodbox.dhall` — Tier-0 binary context, GENERATED (`writeTier0` / `ensureBasicsFloor`),
  self-contained, non-secret, git-ignored.
- `prodbox-config-types.dhall` + `test-secrets-types.dhall` — GENERATED schemas, git-ignored
  ...
  Sprint `1.41`).
- `docker/default-prodbox.dhall` — GENERATED at image-build time (rendered from
  `renderDaemonContainerDefaultDhall` into the build context before `docker build`), git-ignored.
...
|---|---|---|
| Host CLI (`prodbox` on the operator host) | Tier-0 `./prodbox.dhall` (GENERATED, self-contained non-secret binary context AND sealed-Vault bootstrap floor AND the operator seed config in its `parameters`, host file beside the binary). The standalone `./prodbox-config.dhall` seed file is RETIRED (Sprint `1.42`). | `src/Prodbox/Repo.hs::canonicalConfigPaths` + `src/Prodbox/Settings.hs::loadConfigForSettingsWith`; the binary reads `prodbox.dhall`, projects the basics via `projectBasics`, and decodes `( prodbox.dhall ).parameters` (`loadConfigFile`) as the seed; before establishment (no unlock bundle) it uses those `parameters`, and once established it fetches/decrypts the in-force MinIO envelope (Tier 2) via Vault |
| In-cluster gateway daemon | `/etc/gateway/config` (Tier-0 `prodbox.dhall`) | container ships a DEFAULT Tier-0 `prodbox.dhall` (`docker/default-prodbox.dhall`, GENERATED at image-build time, not committed), OVERWRITTEN by the chart-side `gateway-config-<nodeId>` ConfigMap mount; see [helm_chart_platform_doctrine.md](./helm_chart_platform_doctrine.md) and [distributed_gateway_architecture.md](./distributed_gateway_architecture.md) |
...
git-ignored (one-time operator `git rm --cached` untracks them; Sprint `1.41`), as is the
image-build-time `docker/default-prodbox.dhall`. No `.dhall` surface is version-controlled (§0).
...
- A version-controlled `.dhall` surface. Every Dhall file is GENERATED or locally authored and
  git-ignored (`prodbox.dhall`, the `*-types.dhall` schemas, `docker/default-prodbox.dhall`,
  `prodbox-config.dhall`, `test-secrets.dhall`); none is committed (Sprint `1.41`; see §0).
