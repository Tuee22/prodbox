...
not the lifecycle kernel or AWS adapters. `5.35` is Active on the frozen oracle; `5.36` is blocked
by Sprints `4.85` and `5.35` for the `TestRunner` cleanup-client integration. Live substrate
evidence remains deployment qualification.
...
**Status**: Active (opened 2026-08-15). The source operator trace is taken; the stable repository
artifacts, canonical validation, and complete matrix remain.
**Blocked by**: none.
...
Commit the stable named artifacts, implement the pure oracle and complete fault matrix, and remove
the forged residue-absence fixture. Home/AWS destructive campaigns and `TestRunner` client
integration are not closure shortcuts for this sprint; the latter is owned by Sprint `5.36`.

## Sprint 5.36: TestRunner Lifecycle Cleanup Client [⏸️ Blocked]

**Status**: Blocked by Sprints `4.85` and `5.35` (opened 2026-08-15).
**Blocked by**: Sprint `4.85` for the lifecycle-owned durable kernel and Sprint `5.35` for the
canonical teardown oracle. Both are earlier/same-phase dependencies permitted by Standard N.
...
**Implementation**: `src/Prodbox/TestRunner.hs`, `src/Prodbox/TestValidation.hs`, and
`src/Prodbox/ControlPlane/CleanupRunClient.hs`; consume the lifecycle kernel delivered by `4.85`
and remove the validation-owned execution projection it supersedes.
...
Blocked until `4.85` supplies the lifecycle kernel and `5.35` supplies the stable oracle. Then wire
the client, remove the test-owned execution projection, and prove the fake interruption matrix.
