...
cross-language types. The registry entries are:
...
| Resource Lifecycle Classes table in `DEVELOPMENT_PLAN/substrates.md`, sourced from `Prodbox.Lifecycle.ResourceRegistry` | `resource-lifecycle-classes` | Sprint 4.22 (**landed** — markers present in `substrates.md`, generated by `prodbox dev docs generate`) |
| Stack registry↔CLI-command table in `DEVELOPMENT_PLAN/substrates.md`, sourced from `Prodbox.Infra.StackDescriptor` | `stack-command-surface` | Sprint 4.27 (**landed** — markers present in `substrates.md`, generated by `prodbox dev docs generate`) |
| Cross-language types (TypeScript / Go / PureScript mirrors) | `cross-language-types.*` | **Deferred** — no non-Haskell consumer in scope |

The `resource-lifecycle-classes` and `stack-command-surface` entries **have landed**: their marker
pairs are physically present in
[`DEVELOPMENT_PLAN/substrates.md`](../DEVELOPMENT_PLAN/substrates.md) and are regenerated by
`prodbox dev docs generate`. The marker pairs whose home is **this file** (the
`documentation-standards.*` index) are **not yet physically present** here, so this document's header
correctly declares `**Generated sections**: none` — the header must match the markers actually
present in this file, not entries whose markers live in other files.
