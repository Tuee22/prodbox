...
| Public-host proof | `prodbox host public-edge`, `prodbox test integration public-dns`, `prodbox test integration charts-api`, `prodbox test integration charts-websocket`, and the named admin-route validations on the single-host Gateway API target doctrine, including Keycloak shared-host issuer proof, route-level RBAC proof, real WebSocket upgrade behavior, one-connection-per-pod lifetime, revocation-driven reconnect, and readiness-based drain |
