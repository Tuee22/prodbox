...
**Current head state (2026-07-30 — Phase `5` reclosed on Sprint `5.24`, the restore-time gateway
observability wait).** Sprint `5.24` is an own-surface Phase-5 reopen (Standard A) that fixes a
...
| 5 | Canonical Test Suite | 🔄 **Reopened; Sprint `5.24` is ✅ Done** (own-surface reopen of the Sprint `5.16` gateway stability gate: the restore-time sample now waits out a fresh-Pod metrics-scrape gap instead of latching it as fatal — a live-surfaced harness defect, distinct from the heap-leak the fix now correctly surfaces). Sprint `5.23` is ✅ Done on its code-owned surface (forced-drain/finalizer shutdown model + residue oracle; live full-suite contention is the Standard-O axis). Existing `5.21` calibration and `5.22` certificate-scope work remain open. | Capability-bound preparation, always-run cleanup DAG, CPU/queue/deadline/fault oracle, derived restore graph, calibration recorder, certificate-scope serving validation, shutdown-race/residue oracle, restore-time gateway observability wait |
