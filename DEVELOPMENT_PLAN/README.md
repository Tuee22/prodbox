...
**2026-07-06 — Phases `1`, `3`, `4`, and `7` reopened to make the bootstrap readiness-race class
unrepresentable.** Operator decision after a reproducible `cluster reconcile` failure at the
image-mirror step (`dial tcp: lookup minio.prodbox.svc.cluster.local: no such host`): the whole
*class* of bootstrap readiness races must be designed out, not retried. Root cause is a **shallow
gate** — the registry was gated `Ready` by the front-door `GET /v2/` probe, which `registry:2`
serves without touching its MinIO S3 backend, so the first operation to exercise the registry→MinIO
write edge was the mirror push itself; a compounding retry-classifier hole treated `no such host`
as non-retryable. The invariant (new SSoT
[bootstrap_readiness_doctrine.md](../documents/engineering/bootstrap_readiness_doctrine.md)): a step
may run only behind a barrier that exercises the exact dependency call path it uses, bootstrap
ordering is a pure projection over a typed config-sourced component dependency/readiness graph, and
`Unreachable` readiness gates closed — so a readiness race is not a well-formed config value. Owning
sprints (forward-only blocking, Standard N): Sprint `1.56` is ✅ **Done** (2026-07-06 — the typed
`ReadinessProbe` deep/proxy ranking, the `ComponentId`/`ComponentNode` graph carried in the Tier-0
`components` field, and the pure `EffectDAG.acyclicTopologicalOrder` lowering + four graph-validity
rejections; unit 1205/1205, `dev check` 0, CLI/env integration green); Sprint `3.23` is ✅ **Done**
(2026-07-06 — `resolveDependencyOrder` sources chart order from the component graph, the hardcoded
`chartDefinitionDependencies` / `ChartRequiresPatroniPlatform` surfaces are removed, and the Percona
operator gate now proves the Deployment `Available` rather than merely present; unit 1209/1209,
`dev check` 0, `dev lint chart` 0, CLI/env integration green); Sprint `4.43` is ✅ **Done**
(2026-07-06 — narration + execution now project from ONE typed `ReconcileStepId` step table with a
pure graph-consistency check, retiring the two hand-synced lists and closing their live drift; the
deep `ensureRegistryStorageBackendEdgeReady` gate exercises the registry's own S3 write path before
the mirror push; the retry classifier treats `no such host`/`dial tcp`/`lookup`/`name resolution` as
retryable; unit 1214/1214, `dev check` 0, CLI/env integration green, plan goldens refreshed); Sprint
`7.31` is ✅ **Done** on its code-owned surface (2026-07-06 — `ensureAwsSubstratePlatformRuntime` runs
the same deep `ensureRegistryStorageBackendEdgeReady` gate before the EKS image-mirror Job and crane
pushes, and `applyEksImageMirrorJob` re-applies the Job on an `isRetryableEksImageMirrorFailure`
transient; unit 1216/1216, `dev check` 0, CLI/env integration green; the live `--substrate aws` run
is the non-blocking Standard O axis). **The 2026-07-06 bootstrap readiness-race reopening is fully
closed** — the whole class is now unrepresentable across both substrates. Validation gates on each sprint's Independent
Validation line (fake-boundary unit + `prodbox test integration cli`/`env`, `prodbox dev check`); the
green home `prodbox test all` past the image-mirror step and the `--substrate aws` run are
non-blocking Standard O `🧪 Live-proof: pending` axes. Retired surfaces —
the hand-written `runSequentially` bring-up list + `renderNativeInstallPlan` STEP narration, the
hardcoded `chartDefinitionDependencies` / `ChartRequiresPatroniPlatform` edges, and the `/v2/`-only
registry gates — are recorded as Pending Removal rows in
[legacy-tracking-for-deletion.md](legacy-tracking-for-deletion.md).
...
writing any provider secrets to `prodbox.dhall`. The ephemeral elevated/admin credential
enters only through the interactive `SecretRef.Prompt` arm (the test harness simulates that prompt
from `test-secrets.dhall`'s `aws_admin_for_test_simulation.*` fixture); it is never stored in
`prodbox.dhall`, Vault, or on disk. Sprint `7.16` moved the `aws_admin_for_test_simulation`
fixture out of production config and unified admin acquisition on the prompt. 🧪 **Live-proof: pending** — the live first-touch migration/deletion proof and the
...
`.data/`-backed PV (preserved across cluster wipes exactly like MinIO's); `prodbox.dhall`
...
integration cli/env only the 2 pre-existing `charts reconcile vscode` environmental failures (the
...
`aws-ses`, it is never reconciled by `prodbox cluster delete` or `prodbox aws teardown` and is
...
the current `prodbox aws stack aws-ses reconcile|destroy --yes` CLI surface; the
`ses : { sender_domain, receive_subdomain, capture_bucket }` block in
`prodbox-config-types.dhall` and `prodbox.dhall`; Keycloak realm chart updates
...
| AWS | `prodbox aws stack eks reconcile` + `prodbox aws stack aws-subzone reconcile` + `prodbox aws stack test reconcile` | `prodbox aws stack aws-subzone destroy --yes` + `prodbox aws stack eks destroy --yes` + `prodbox aws stack test destroy --yes` | ✅ Phase 7-owned AWS substrate parity proved live June 5-9, 2026 for the then-canonical AWS slice, including public DNS, chart validations, admin routes, `keycloak-invite`, destructive lifecycle, and per-run postflight cleanup. Current canonical-suite membership is defined in `src/Prodbox/TestPlan.hs`; any AWS live proofs for later-added validations are tracked only in [substrates.md](substrates.md)'s per-validation coverage table as non-blocking Standard O axes. | [phase-7-aws-substrate-foundations.md → Sprint 7.5](phase-7-aws-substrate-foundations.md) |
...
test-simulation fixture out of production config into the test-harness-only `test-secrets.dhall`
and unifies elevated/admin AWS acquisition on the interactive `SecretRef.Prompt`, so
`prodbox.dhall` carries no admin block and the generated operational `aws.*` is minted into
