...
total graph, and authored capacity becomes measured-certified. Foundation Epoch Sprints
...
| Date | Milestone |
|------|-----------|
| 2026-07-23 | Sprint `4.48` 🔄 **Active — Increment A (the pure genesis admission fold) landed; Phase `4` opens on `4.48`** — `src/Prodbox/Lifecycle/Authority/Genesis.hs` is the pure `GenesisFrozen → EstablishAuthorityBackup → BackupEstablished` fold: `AuthorityAdmissionState` + `AuthorityGenesisCommand`/`GenesisDecision`/`AuthorityGenesisEvent` with total `decideGenesis`/`evolveGenesis`/`stepGenesis`, modeled on `ControlPlane.Capacity`. It enforces the genesis invariant — normal admission opens ONLY after BOTH the home Target Agent generation receipt AND the Authority Backup Adapter receipt are read back, under the genesis `AuthorityEpoch` — refuses out-of-phase commands, is plan-idempotent (divergent plan refused), and replays events idempotently. Unblocked by Sprint `3.26`'s landed control-plane charts. Evidence: `prodbox dev check` exit 0; `test/unit/LifecycleAuthorityGenesis.hs` 8/8. Remaining 4.48: the durable `OperationRecord`/outbox (generalizing `Broker.RequestJournal`), the `AuthorityState` aggregate over the three retained projections, the host-direct interpreter (on `AuthorityObjectCore` + `ControlPlane.AuthorityClock`, replacing the gateway-hosted transport/clock), the `BackupRepair` reopen, and the `Lease*`/`CheckpointAuthority*`/`TargetCommit*` migrations. |
...
| 1 | Haskell Runtime, CLI, Config, and Pulumi Foundations | ✅ **Reclosed on Sprint `1.67`.** Sprints `1.61`–`1.66` are Done; substrate-neutral `K8sClusterReachable` now proves the selected cluster through `ToolKubectl` + `kubectl cluster-info`, while local RKE2 file/service facts remain explicit home-local nodes. | Operation-indexed capabilities, exact graph requirements, absolute deadlines, service-capacity algebra, native object-store and managed Vault-session boundaries, conformance tier and legacy escape registry, measured capacity certification, and substrate-neutral prerequisite topology |
...
| 3 | Haskell Chart Platform and Public Workload Delivery | 🔄 **Reopened and Active; Sprint `3.26` Increments A + B + C + D + E + F landed** (F: `vaultIdentityRegistryViolations` compiled cross-check + dev-check conformance) (A: Bootstrap Broker distinct Vault role + typed chart statics; B: the `charts/bootstrap-broker/` workload chart + generated-section drift gate + chart-lint + tests; C: capacity plan reserves a `bootstrap-broker` quota + profile funded by an operator-approved vscode trim, `ChartPlatform` renders the broker; D: `ComponentGraph` chart-only `ComponentChartBootstrapBroker` node + bijection, so the plan builder drives the broker without changing production topology; E: `charts/gateway` converts per-node Deployments → stable per-node **StatefulSets** with retained emitter-journal volumes (home hostPath / AWS `ReadWriteOncePod` PVC) + Lease RBAC, verified by in-session `helm template` on both substrates). Remaining: the Standard-P pre-Vault cutover, and the other control-plane roles (Authority, adapters, provider worker, target agent, permit Jobs) with their Vault inventory. | Separate broker/authority/agent workloads, identities, policies, probes, retained journals, and resource envelopes |
| 4 | Lifecycle Hardening, Pulumi Decoupling, and Python Removal | 🔄 **Reopened; Sprint `4.48` Active** (Increment A — the pure genesis admission fold `GenesisFrozen → EstablishAuthorityBackup → BackupEstablished` — landed + validated 2026-07-23; unblocked by Sprint `3.26`'s landed control-plane charts). Foundation Epoch Sprint `4.51` is also Active (Increment A landed 2026-07-14; Increment B cutover deferred). Sprints `4.49`–`4.50` follow `4.48`. | Durable Lifecycle Authority, immutable checkpoints, operation journal/outbox, target delivery, authority-epoch cutover, removal of gateway/host-direct authority, durability-indexed retained authority storage |
...
Phases `0`, `1`, and `2` are reclosed: Phase `0` on Sprint `0.17` (with the later governance-only
Sprint `0.18`), Phase `1` on Sprint `1.67`, and Phase `2` on Sprint `2.33`. Sprints `1.61`–`1.66`
are Done, and Sprint `1.67` removes the substrate leak from the generic Kubernetes prerequisite
closure. Sprint `2.33` extracts pre-Vault recovery into a minimal Bootstrap Broker and cuts the
...
`5.21` retain their own Active status. Sprints `1.61` and `1.62` are Done, and Sprint `1.67`
recloses their phase after removing the substrate-specific prerequisite edge. The
