...
**Current head state (2026-08-08, second entry — the doctrine is corrected; Phases `4` and `5`
...
`cabal build … all` resolves to `lib` and `exe:prodbox` with **no test suite**, so for `test/` —
...
expanded owned surfaces. Their exact closure states are summarized in the Phase Overview and
defined in the phase files; no later phase reopens an earlier one. Production remains on the
