...
-- | Provider-owned implementation of the EKS client-auth envelope.
--
-- Raw bearer construction and envelope sealing live only in this hidden
-- module.  The public facade exposes destination preparation, canonical
-- decoding/opening, and read-only projection accessors.
module Prodbox.ControlPlane.EksClientAuthProjection.Internal
...
mkEksClientAuthProjection account region cluster clusterArn endpoint caData bearer expires = do
...
eksClientAuthClusterArn = internalClusterArn
...
openEksClientAuthProjection destination envelope = do
  ...
  projection <-
    first
      (const EksClientAuthEnvelopeInvalid)
...
validate field maximumLength value
  | Text.null value || Text.length value > maximumLength =
      Left (EksClientAuthFieldInvalid field)
...
validateEksClusterArnBinding
  ...
  -> Text
  -> Text
  -> Either EksClientAuthProjectionError ()
validateEksClusterArnBinding account region cluster rawArn =
  case Text.splitOn ":" rawArn of
    ["arn", partition, "eks", arnRegion, arnAccount, resource]
      | partition `elem` ["aws", "aws-cn", "aws-us-gov"]
      , arnRegion == region
      , arnAccount == account
      , resource == "cluster/" <> cluster ->
          Right ()
    _ -> Left (EksClientAuthFieldInvalid "cluster-arn")
