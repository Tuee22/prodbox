...
| [acme_provider_guide.md](./acme_provider_guide.md) | ZeroSSL ACME guidance for the interactive onboarding flow, and the configurable `CertScopeSet` certificate-scope model — exact and wildcard scopes, coverage/admission semantics, exact-scope retention, and the edge projections derived from the one scope set |
...
| [shared_host_resource_protocol.md](./shared_host_resource_protocol.md) | Not-adopted record of how prodbox would participate in the operator-installed host claim ledger: one `Persistent` claim covering the retained local control plane, charges derived from the already-compiled resource plan rather than authored again, attachment at the existing observed-host seam, and the standing gap that a direct `systemctl start rke2-server` bypasses any cooperative claim |
...
- [Shared Host Resource Protocol](./shared_host_resource_protocol.md)

### Secrets and Vault

- [Vault Secret-Management Doctrine](./vault_doctrine.md)
- [Burn Recipient Provenance](./burn_recipient_provenance.md)
